"""
Kamera/ölçek kalibrasyon verisi toplama aracı.

old_docs/calibration_click_measure.py'nin davranış değiştirilmeden taşınmış
hâlidir. Tek fark: CSV fieldnames artık `calibration.schema.FIELDNAMES`'ten
geliyor (bkz. schema.py'deki açıklama — eski koddaki başlık/veri sütun
kayması hatasının yeni toplanacak verilerde tekrarlanmaması için).

picamera2 importu, eski dosyada olduğu gibi burada da yalnızca bu dosyaya
özgüdür (donanım gerektiren bir kalibrasyon aracıdır).
"""
...
def draw_header(frame, target_size_m, distance_m, angle_deg, sample_count, mode_text):
    ...
    for line in lines:
        cv2.putText(frame, line, (15, y), cv2.FONT_HERSHEY_SIMPLEX, 0.52, (255, 255, 255), 2)
        y += 27

    if time.time() < status_until and status_message:
        cv2.rectangle(frame, (0, HEIGHT - 55), (WIDTH, HEIGHT), (0, 80, 0), -1)
        cv2.putText(
            frame, status_message, (15, HEIGHT - 20),
...
def redraw_points():
    ...
    for i, p in enumerate(points):
        ...
        cv2.putText(
            display_frame, str(i + 1), (p[0] + 8, p[1] - 8),
    ...
    if len(points) == 4:
        ...
        cv2.circle(display_frame, (target_center_x, target_center_y), 7, (255, 0, 255), -1)
        cv2.line(
            display_frame,
...
def main():
    ...
    config = picam2.create_preview_configuration(
        ...
        controls={"FrameRate": FPS, "AwbEnable": True, "AeEnable": True}
    )
    ...
    while True:
        if frozen_frame is None:
            ...
            draw_header(frame_bgr, target_size_m, distance_m, angle_deg, sample_count, "LIVE CAMERA")
            ...
            if display_frame is not None:
                ...
                draw_header(
                    frame_to_show, target_size_m, distance_m, angle_deg, sample_count,
        ...
        if key == ord("s"):
            ...
            success, message = delete_last_sample_for_scenario(target_size_m, distance_m, angle_deg)
            sample_count = count_existing_measurements(target_size_m, distance_m, angle_deg)
            ...
            if not center_ok:
                set_status(f"[WARN] Center BAD dx={dx}px dy={dy}px. Yine de kaydedildi.", 3.0)
